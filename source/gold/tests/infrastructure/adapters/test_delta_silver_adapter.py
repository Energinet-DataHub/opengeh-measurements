...
def test__read_stream__should_contain_rows_in_silver(spark: SparkSession, create_silver_tables):
    ...
    df_silver.write.format("delta").mode("append").saveAsTable(f"{catalog_settings.silver_database_name}.{table_name}")
    ...
    result.writeStream.format("delta").option("checkpointLocation", "/tmp/checkpoints/read_stream_test").foreachBatch(
        ...
        .mode("append")
        .saveAsTable(f"{catalog_settings.silver_database_name}.{test_table}")
    ).trigger(once=True).start().awaitTermination()

    # Assert
    assert (
        spark.read.table(f"{catalog_settings.silver_database_name}.{test_table}")
