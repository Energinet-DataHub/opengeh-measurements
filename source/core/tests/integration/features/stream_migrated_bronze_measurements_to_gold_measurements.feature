Feature: Streaming Migrated Bronze Measurements to Gold Measurements
  ...
  Scenario: Migrated transactions from Bronze to Gold
    ...
    Then 24 migrated measurements row(s) are available in the gold measurements table
  ...
  Scenario: Filtering migrated transactions before 2017
    ...
    When streaming migrated transactions to the Gold layer
    Then 0 migrated measurements row(s) are available in the gold measurements table
  ...
  Scenario: Migrating quality of type QM always returns null in quantity
