Feature: Current_sap_v1 Gold View

  Scenario: View has expected schema
    When accessing the current_sap_v1 gold view
  ...
  Scenario: View returns only the latest active (not cancelled) measurement
    ...
    When querying the current_sap_v1 gold view for that metering point and expected quantity
  ...
  Scenario Outline: A gold measurement with null <column> is excluded from current_sap_v1 view
  ...
  Scenario: A gold measurement with quantity being null is included in current_sap_v1 view
