...
import core.bronze.application.streams.notify_transactions_persisted_stream as sut


@mock.patch("core.bronze.application.config.spark_session.initialize_spark")
@mock.patch(
    "core.bronze.domain.transformations.submitted_transactions_transformation.create_by_packed_submitted_transactions"
)
@mock.patch(
    "core.bronze.domain.transformations.transactions_persisted_events_transformation.create_by_unpacked_submitted_transactions"
)
@mock.patch("core.bronze.infrastructure.streams.bronze_repository.BronzeRepository.read_submitted_transactions")
@mock.patch("core.bronze.infrastructure.streams.kafka_stream.KafkaStream.write_stream")
def test__notify__should_call_expected(
    mock_kafka_stream_write_stream,
    mocked_bronze_repository_read_submitted_transactions,
    ...
    mocked_bronze_repository_read_submitted_transactions.return_value = mock_bronze_repository
    ...
    mocked_bronze_repository_read_submitted_transactions.assert_called_once()
    mock_submitted_transactions_transformation.assert_called_once_with(mock_bronze_repository)
