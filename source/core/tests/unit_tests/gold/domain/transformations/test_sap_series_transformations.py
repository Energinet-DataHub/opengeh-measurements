...
def test__transform_calculated__should_match_measurements_sap_series_schema(spark: SparkSession) -> None:
    # Arrange
    silver_measurements = CalculatedMeasurementsBuilder(spark).add_row().build()

    # Act
    actual = sut.transform_calculated(silver_measurements)
