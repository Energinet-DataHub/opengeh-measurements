...
def test__append_if_not_exists__when_multiple_clustering_columns_to_filter__calls_expected(
    ...
    expected_condition = "current.id <=> update.id AND ((CAST(current.clustering_column_1 AS DATE) >= '2023-01-01' AND CAST(current.clustering_column_1 AS DATE) <= '2023-01-02')) AND (CAST(current.clustering_column_2 AS DATE) == '2023-01-02' OR CAST(current.clustering_column_1 AS DATE) == '2023-01-02'))"
