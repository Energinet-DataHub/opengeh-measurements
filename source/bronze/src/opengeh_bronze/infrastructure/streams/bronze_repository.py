...
class BronzeRepository:
    ...
    def read_measurements(self) -> DataFrame:
        return self.spark.readStream.format("delta").table(
            f"{self.catalog_settings.bronze_database_name}.{TableNames.bronze_measurements_table}"
        )
    ...
    def read_submitted_transactions(self) -> DataFrame:
        return (
            ...
            .table(f"{self.catalog_settings.bronze_database_name}.{TableNames.bronze_submitted_transactions_table}")
        )
