...
DEFAULT_CALCULATION_PERIOD_START = datetime(2024, 2, 1, 23)
DEFAULT_CALCULATION_PERIOD_END = datetime(2024, 3, 1, 23)
...
def _get_contract_parameters(filename: str) -> list[str]:
    with open(filename) as file:
        ...
        text = text.replace("{period-start}", DEFAULT_CALCULATION_PERIOD_START.strftime("%Y-%m-%dT%H:%M:%SZ"))
        text = text.replace("{period-end}", DEFAULT_CALCULATION_PERIOD_END.strftime("%Y-%m-%dT%H:%M:%SZ"))
...
def test_when_parameters__parses_parameters_from_contract(
    ...
    assert actual_args.calculation_period_start == DEFAULT_CALCULATION_PERIOD_START
    assert actual_args.calculation_period_end == DEFAULT_CALCULATION_PERIOD_END
