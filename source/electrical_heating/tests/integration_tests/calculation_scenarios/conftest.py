...
from .temp_testcommon import TestCases, TestCase
...
def test_cases(spark: SparkSession, request) -> TestCases:
    ...
    consumption_metering_point_periods = read_csv(
        spark,
        f"{scenario_path}/when/electrical_heating_internal/consumption_metering_point_periods.csv",
    )
    child_metering_point_periods = read_csv(
        spark,
        f"{scenario_path}/when/electrical_heating_internal/child_metering_point_periods.csv",
    )

    # Execute the calculation logic
    actual_measurements = _execute(
        time_series_points,
    ...
    # Return test cases
    return TestCases(
        [TestCase("measurements", actual_measurements, expected_measurements)]
    )
