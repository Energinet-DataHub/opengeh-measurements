...
from opengeh_calculated_measurements.opengeh_electrical_heating.domain.column_names import (
    ColumnNames,
)
from opengeh_calculated_measurements.opengeh_electrical_heating.domain.constants import (
    ELECTRICAL_HEATING_LIMIT_YEARLY,
)
from opengeh_calculated_measurements.opengeh_electrical_heating.domain.types import (
    NetSettlementGroup,
)
...
def execute(
    ...
    energy = time_series_points.where(
        (
            ...
            == MeteringPointType.CONSUMPTION_METERING_POINT_TYPE.value
        )
        | (
            F.col(ColumnNames.metering_point_type)
    ...
    energy = _filter_parent_child_overlap_period_and_year(
        metering_point_periods_with_energy
    )
        ...
        .join(
            ...
            (
                ...
                & (
                    ...
                    == F.col(f"previous.{_CalculatedNames.date}")
                )
                & (
                    F.col(f"current.{ColumnNames.quantity}")
                    == F.col(f"previous.{ColumnNames.quantity}")
                )
            ),
            "left_anti",
        )
...
def _aggregate_quantity_over_period(time_series_points: DataFrame) -> DataFrame:
    ...
    return time_series_points.select(
        ...
        .over(period_window)
        .alias(_CalculatedNames.cumulative_quantity),
        F.col(ColumnNames.metering_point_id),
        F.col(_CalculatedNames.date),
        F.col(ColumnNames.quantity),
        F.col(_CalculatedNames.period_energy_limit),
...
def _filter_parent_child_overlap_period_and_year(
    ...
    return time_series_points.where(
        ...
        & (
            F.year(F.col(_CalculatedNames.date))
            == F.year(F.col(_CalculatedNames.period_year))
        )
    )
...
def _join_source_metering_point_periods_with_energy(
    ...
    return (
        ...
        .join(
            ...
            == F.col(
                f"metering_point.{_CalculatedNames.energy_source_metering_point_id}"
            ),
            "inner",
        )
        .select(
            F.col(f"metering_point.{_CalculatedNames.parent_period_start}").alias(
                _CalculatedNames.parent_period_start
            ),
            F.col(f"metering_point.{_CalculatedNames.parent_period_end}").alias(
                _CalculatedNames.parent_period_end
            ),
            F.col(
                f"metering_point.{_CalculatedNames.electrical_heating_metering_point_id}"
            ...
            F.col(f"metering_point.{_CalculatedNames.period_energy_limit}").alias(
                _CalculatedNames.period_energy_limit
            ),
            F.col(f"metering_point.{_CalculatedNames.overlap_period_start}").alias(
                _CalculatedNames.overlap_period_start
            ),
            F.col(f"metering_point.{_CalculatedNames.overlap_period_end}").alias(
                _CalculatedNames.overlap_period_end
            ),
        )
    )
...
def _calculate_daily_quantity(time_series: DataFrame) -> DataFrame:
    ...
    return (
        time_series.select(
            "*",
            F.date_trunc("day", F.col(ColumnNames.observation_time)).alias(
                _CalculatedNames.date
            ),
        )
        .select(
            F.sum(F.col(ColumnNames.quantity))
...
def _split_period_by_year(
    ...
    return parent_and_child_metering_point_and_periods.select(
        ...
        F.col(_CalculatedNames.parent_net_settlement_group),
        F.when(
            F.year(F.col(_CalculatedNames.parent_period_start))
            == F.year(F.col(_CalculatedNames.period_year)),
            F.col(_CalculatedNames.parent_period_start),
        )
        ...
        F.when(
            F.year(F.col(_CalculatedNames.parent_period_end))
            == F.year(F.col(_CalculatedNames.period_year)),
            F.col(_CalculatedNames.parent_period_end),
        )
        .otherwise(
            begining_of_year(date=F.col(_CalculatedNames.period_year), years_to_add=1)
        )
    ...
    return (
        ...
        .select(
            ...
            F.col(f"parent.{ColumnNames.net_settlement_group}").alias(
                _CalculatedNames.parent_net_settlement_group
            ),
            ...
            F.col(f"net_consumption.{ColumnNames.coupled_date}").alias(
                _CalculatedNames.net_consumption_period_start
            ),
            F.col(f"net_consumption.{ColumnNames.uncoupled_date}").alias(
                _CalculatedNames.net_consumption_period_end
            ),
        )
    )
