...
from opengeh_calculated_measurements.opengeh_electrical_heating.infrastructure.electricity_market.child_metering_points.wrapper import (
    ChildMeteringPoints,
)
...
from opengeh_calculated_measurements.opengeh_electrical_heating.infrastructure.electricity_market.consumption_metering_point_periods.wrapper import (
...
class Repository:
    ...
    def read_consumption_metering_point_periods(
        self,
    ) -> ConsumptionMeteringPointPeriods:
        ...
        df = _read_csv(
            spark=self._spark,
            path=file_path,
            schema=consumption_metering_point_periods_v1,
        )
    ...
    def read_child_metering_points(self) -> ChildMeteringPoints:
        ...
        df = _read_csv(
            spark=self._spark, path=file_path, schema=child_metering_points_v1
        )
