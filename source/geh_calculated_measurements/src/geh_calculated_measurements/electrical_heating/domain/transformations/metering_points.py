...
def _join_children_to_parent_metering_point(
    ...
    return (
        ...
        .select(
            ...
            F.col(f"parent.{ColumnNames.period_from_date}").alias(EphemiralNames.parent_period_start),
            F.col(f"parent.{ColumnNames.period_to_date}").alias(EphemiralNames.parent_period_end),
            F.col(f"electrical_heating.{ColumnNames.metering_point_id}").alias(
                EphemiralNames.electrical_heating_metering_point_id
            ),
            F.col(f"electrical_heating.{ColumnNames.coupled_date}").alias(
                EphemiralNames.electrical_heating_period_start
            ),
            F.col(f"electrical_heating.{ColumnNames.uncoupled_date}").alias(
                EphemiralNames.electrical_heating_period_end
            ),
            F.col(f"net_consumption.{ColumnNames.metering_point_id}").alias(
                EphemiralNames.net_consumption_metering_point_id
            ),
            F.col(f"net_consumption.{ColumnNames.coupled_date}").alias(EphemiralNames.net_consumption_period_start),
            F.col(f"net_consumption.{ColumnNames.uncoupled_date}").alias(EphemiralNames.net_consumption_period_end),
            F.col(f"consumption_from_grid.{ColumnNames.metering_point_id}").alias(
                EphemiralNames.consumption_from_grid_metering_point_id
            ),
            F.col(f"consumption_from_grid.{ColumnNames.coupled_date}").alias(
                EphemiralNames.consumption_from_grid_period_start
            ),
            F.col(f"consumption_from_grid.{ColumnNames.uncoupled_date}").alias(
                EphemiralNames.consumption_from_grid_period_end
            ),
            F.col(f"supply_to_grid.{ColumnNames.metering_point_id}").alias(
                EphemiralNames.supply_to_grid_metering_point_id
            ),
            F.col(f"supply_to_grid.{ColumnNames.coupled_date}").alias(EphemiralNames.supply_to_grid_period_start),
            F.col(f"supply_to_grid.{ColumnNames.uncoupled_date}").alias(EphemiralNames.supply_to_grid_period_end),
        )
    )
...
def _close_open_ended_periods(
    ...
    return parent_and_child_metering_point_and_periods.select(
        ...
        F.col(EphemiralNames.parent_period_start),
        F.coalesce(
            F.col(EphemiralNames.parent_period_end),
            "end_of_year",
        ).alias(EphemiralNames.parent_period_end),
        # Electrical heating metering point
        F.col(EphemiralNames.electrical_heating_period_start),
        F.coalesce(
            F.col(EphemiralNames.electrical_heating_period_end),
            "end_of_year",
        ).alias(EphemiralNames.electrical_heating_period_end),
        F.col(EphemiralNames.electrical_heating_metering_point_id),
        # Net consumption metering point
        F.col(EphemiralNames.net_consumption_period_start),
        F.coalesce(
            F.col(EphemiralNames.net_consumption_period_end),
            "end_of_year",
        ).alias(EphemiralNames.net_consumption_period_end),
        F.col(EphemiralNames.net_consumption_metering_point_id),
        # Consumption from grid metering point
        EphemiralNames.consumption_from_grid_metering_point_id,
        EphemiralNames.consumption_from_grid_period_start,
        F.coalesce(EphemiralNames.consumption_from_grid_period_end, "end_of_year").alias(
            EphemiralNames.consumption_from_grid_period_end
        ),
        # Supply to grid metering point
        EphemiralNames.supply_to_grid_metering_point_id,
        EphemiralNames.supply_to_grid_period_start,
        F.coalesce(EphemiralNames.supply_to_grid_period_end, "end_of_year").alias(
            EphemiralNames.supply_to_grid_period_end
        ),
    )
...
def _find_parent_child_overlap_period(
    ...
    return parent_and_child_metering_point_and_periods.select(
        ...
        F.greatest(
            ...
            F.col(EphemiralNames.electrical_heating_period_start),
            F.col(EphemiralNames.net_consumption_period_start),
            F.col(EphemiralNames.consumption_from_grid_period_start),
            F.col(EphemiralNames.supply_to_grid_period_start),
        ).alias(EphemiralNames.overlap_period_start),
        F.least(
            F.col(EphemiralNames.parent_period_end),
            F.col(EphemiralNames.electrical_heating_period_end),
            F.col(EphemiralNames.net_consumption_period_end),
            F.col(EphemiralNames.consumption_from_grid_period_end),
            F.col(EphemiralNames.supply_to_grid_period_end),
        ).alias(EphemiralNames.overlap_period_end),
    ).where(F.col(EphemiralNames.overlap_period_start) < F.col(EphemiralNames.overlap_period_end))
...
def _split_period_by_year(
    ...
    return parent_and_child_metering_point_and_periods.select(
        ...
        F.explode(
            F.sequence(
                begining_of_year(F.col(EphemiralNames.parent_period_start)),
                F.coalesce(
                    # Subtract a tiny bit to avoid including the next year if the period ends at new year
                    begining_of_year(F.expr(f"{EphemiralNames.parent_period_end} - INTERVAL 1 SECOND")),
        ...
        F.col(ColumnNames.net_settlement_group),
        F.when(
            F.year(F.col(EphemiralNames.parent_period_start)) == F.year(F.col("period_year")),
            F.col(EphemiralNames.parent_period_start),
        )
        .otherwise(begining_of_year(date=F.col("period_year")))
        .alias(EphemiralNames.parent_period_start),
        F.when(
            F.year(F.col(EphemiralNames.parent_period_end)) == F.year(F.col("period_year")),
            F.col(EphemiralNames.parent_period_end),
        )
        .otherwise(begining_of_year(date=F.col("period_year"), years_to_add=1))
        .alias(EphemiralNames.parent_period_end),
        F.col(EphemiralNames.overlap_period_start),
        F.col(EphemiralNames.overlap_period_end),
        F.col(EphemiralNames.period_year),
        F.col(EphemiralNames.electrical_heating_metering_point_id),
        F.col(EphemiralNames.net_consumption_metering_point_id),
        F.col(EphemiralNames.consumption_from_grid_metering_point_id),
        F.col(EphemiralNames.supply_to_grid_metering_point_id),
    )
...
def _remove_nsg2_up2end_without_netcomsumption(
    ...
    return parent_and_child_metering_point_and_periods.where(
        ~(
            ...
            & (F.col(EphemiralNames.net_consumption_metering_point_id).isNull())
            & (F.year(F.col(EphemiralNames.period_year)) == F.year(F.current_date()))
        )
    )
