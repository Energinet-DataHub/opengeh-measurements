...
@start_trace()
def orchestrate_business_logic(job_arguments: MissingMeasurementsLogArgs, logging_settings: LoggingSettings) -> None:
    logger = Logger(__name__)
    logger.info(f"Command line arguments / env variables retrieved for Logging Settings: {logging_settings}")
    logger.info(
        f"Command line arguments retrieved for missing measurements log job Oriented Parameters: {job_arguments}"
    )
