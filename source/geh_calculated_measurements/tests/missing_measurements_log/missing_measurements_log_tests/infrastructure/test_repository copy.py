...
def _create_repository_with_extra_col(spark: SparkSession) -> MeteringPointPeriodsRepository:
    return MeteringPointPeriodsRepository(
        spark,
        SPARK_CATALOG_NAME,
    )


def _create_repository_with_missing_col(spark: SparkSession) -> MeteringPointPeriodsRepository:
    return MeteringPointPeriodsRepository(
        spark,
...
def test__when_consumption_source_contains_unexpected_columns_returns_data_without_unexpected_column(
    ...
    # Act
    consumption_with_extra_input_col = _create_repository_with_extra_col(spark).read_metering_point_periods()
